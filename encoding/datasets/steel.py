...
class SteelSegmentation(BaseDataset):
    NUM_CLASS = 4
    def __init__(self, root=os.path.expanduser('~/.encoding/data'), source=None, split=None,
                 mode=None, transform=None, target_transform=None, **kwargs):
        super(SteelSegmentation, self).__init__(
        ...
        self.uid = list(np.concatenate([np.load(root + '/splited/%s' % f, allow_pickle=True) for f in self.split]))
        if mode == 'train' or self.mode == 'val':
            df = pd.concat([pd.read_csv(root + '/%s' % f) for f in self.source])
